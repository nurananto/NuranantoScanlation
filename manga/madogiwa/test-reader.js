...
// ===========================
// LOAD MANGA DATA FROM GITHUB JSON
// ===========================
async function loadMangaData() {
  try {
    showLoading();
    
    // Load chapters.json dari GitHub repo gambar
    ...
    if (!response.ok) {
      throw new Error('Gagal memuat data chapter dari GitHub');
    }
    
    const data = await response.json();
    
    document.title = data.manga || 'Manga Reader (TEST)';
    
    chapters = data.chapters || [];
    
    console.log(`✅ Loaded ${chapters.length} chapters from GitHub:`, chapters);
...
async function loadChapterImages(chapterNum, pageCount) {
  try {
    ...
    // Base URL ke GitHub repo gambar
    const baseUrl = 'https://raw.githubusercontent.com/nurananto/MadogiwaHenshuutoBakaniSaretaOrega-FutagoJKtoDoukyosuruKotoniNatta/main';
    
    console.log(`Loading ${pageCount} pages for chapter ${chapterNum}...`);
    
    for (let i = 1; i <= pageCount; i++) {
      // Format: /1/Image01.jpg, /1/Image02.jpg
      const pageNum = String(i).padStart(2, '0'); // 1 -> "01", 2 -> "02"
      const imageUrl = `${baseUrl}/${chapterNum}/Image${pageNum}.jpg`;
...
function prevChapter() {
  ...
  if (currentIndex > 0) {
    ...
    const newUrl = `test-reader.html?manga=${mangaId}&chapter=${prevChapter.num}`;
...
function nextChapter() {
  ...
  if (currentIndex < chapters.length - 1) {
    ...
    const newUrl = `test-reader.html?manga=${mangaId}&chapter=${nextChapter.num}`;
    window.history.pushState({}, '', newUrl);
  } else {
...
function generateChapterList() {
  ...
  chapters.forEach(chapter => {
    ...
    div.addEventListener('click', function() {
      ...
      const newUrl = `test-reader.html?manga=${mangaId}&chapter=${chapter.num}`;
      window.history.pushState({}, '', newUrl);
    });
