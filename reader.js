...
function setupEnhancedEventListeners() {
    ...
    // Touch support for mobile - only for progress bar handle
    let touchTimeout;
    ...
    // Touch near bottom edge (within 30px) to show navigation
    document.addEventListener('touchstart', (e) => {
        const windowHeight = window.innerHeight;
        const touchY = e.touches[0].clientY;
        
        // Only show if touching very bottom (within 30px from bottom)
        if (touchY > windowHeight - 30) {
            document.body.classList.add('show-page-nav');
            clearTimeout(touchTimeout);
